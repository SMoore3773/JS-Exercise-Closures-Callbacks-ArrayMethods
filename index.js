...
function processSum(numberList, callback) {
  ...
  return callback(numberList.reduce(function(acc, curVal){return acc+curVal;},0));
}
...
function processProduct(num1, num2, callback) {
  ...
  return callback(numArr.reduce(function(acc, curVal){return acc*curVal;}));
}
...
function getFullNames(runners) {
  const runnersNames =[]
  runners.forEach(i => runnersNames.push(`${i.last_name}, ${i.first_name}`))
  return runnersNames;
}
/**
...
function firstNamesAllCaps(runners) {
  const allCaps = runners.map(i => i.first_name.toUpperCase());
  return allCaps;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const rBSS = runners.filter(i => i.shirt_size === tShirtSize)
  return rBSS;
}
...
function tallyUpDonations(runners) {
  const donTot = runners.reduce((acc, curVal) => acc + curVal.donation, 0)
  return donTot;
}
